#include "../headers/FreeImageHelper.h"
...
FreeImage::FreeImage(unsigned int width, unsigned int height,
                     unsigned int nChannels)
    : w(width), h(height), nChannels(nChannels),
      data(new float[nChannels * width * height]) {}

FreeImage::FreeImage(const FreeImage &img)
    : w(img.w), h(img.h), nChannels(img.nChannels),
      data(new float[nChannels * img.w * img.h]) {
  memcpy(data, img.data, sizeof(float) * nChannels * w * h);
}

FreeImage::FreeImage(const std::string &filename)
    : w(0), h(0), nChannels(0), data(nullptr) {
...
void FreeImage::SetDimensions(unsigned int width, unsigned int height,
                              unsigned int nChannels) {
  if (data != nullptr)
    delete[] data;
  w = width;
...
bool FreeImage::LoadImageFromFile(const std::string &filename,
  ...
  for (int y = 0; y < (int)h; ++y) {
    memcpy(&(data[y * nChannels * w]),
           &bits[sizeof(float) * (h - 1 - y) * nChannels * w],
           sizeof(float) * nChannels * w);
  }
  ...
  for (unsigned int j = 0; j < h; j++) {
    for (unsigned int i = 0; i < w; i++) {
      ...
      for (unsigned int c = 0; c < nChannels && c < 3; ++c) {
        // col[c] = std::min(std::max(0, (int)(255.0f*data[nChannels * (w*j + i)
        // + c])), 255);
        col[c] = std::min(
            std::max(0, (int)(255.0f * data[nChannels * (w * j + i) + c])),
            255);
      }
...
FreeImageB::FreeImageB(unsigned int width, unsigned int height,
                       unsigned int nChannels)
    : w(width), h(height), nChannels(nChannels),
...
FreeImageB::FreeImageB(const FreeImage &img)
    : w(img.w), h(img.h), nChannels(img.nChannels),
      data(new BYTE[nChannels * img.w * img.h]) {
  memcpy(data, img.data, sizeof(BYTE) * nChannels * w * h);
}

FreeImageB::FreeImageB(const std::string &filename)
    : w(0), h(0), nChannels(0), data(nullptr) {
  LoadImageFromFile(filename);
}
...
void FreeImageB::SetDimensions(unsigned int width, unsigned int height,
                               unsigned int nChannels) {
  if (data != nullptr)
    delete[] data;
  w = width;
...
bool FreeImageB::LoadImageFromFile(const std::string &filename,
                                   unsigned int width, unsigned int height) {
  FreeImage_Initialise();
  if (data != nullptr)
    delete[] data;
  ...
  for (int y = 0; y < (int)h; ++y) {
    for (int x = 0; x < (int)w; ++x) {
      for (int c = 0; c < (int)nChannels; ++c) {
        data[(y * w + x) * nChannels + c] =
            (unsigned char)(std::max(
                                std::min(
                                    bitsF[((h - 1 - y) * w + x) * nChannels +
                                          c],
                                    1.0f),
                                0.0f) *
                            255);
...
FreeImageU16F::FreeImageU16F(const std::string &filename)
    : w(0), h(0), nChannels(0), data(nullptr) {
  LoadImageFromFile(filename);
}
...
bool FreeImageU16F::LoadImageFromFile(const std::string &filename,
                                      unsigned int width, unsigned int height) {
  FreeImage_Initialise();
  if (data != nullptr)
    delete[] data;
  ...
  for (int y = 0; y < (int)h; ++y) {
    for (int x = 0; x < (int)w; ++x) {
      for (int c = 0; c < (int)nChannels; ++c) {
        data[(y * w + x) * nChannels + c] =
            bitsF[((h - 1 - y) * w + x) * nChannels + c] * (256 * 256 - 1);
      }
    }
  }
